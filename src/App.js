  ...
  const HeaderGlobal = ({ titulo, color, showBack = true }) => (
    <div style={{...S.globalHeader, borderBottom: `4px solid ${color || C.red}`}}>
      <div style={S.headerLeft}>
        ...
        <h2 style={{margin:0, color: color || C.red, fontSize: 20}}>{titulo}</h2>
      </div>
    ...
    return (
      <div style={S.centerView}>
        <div style={S.loginBox}>
          ...
          <button onClick={() => navigate("/")} style={S.btnText}>Volver</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={S.view}>
      ...
      <div style={S.tabBar}>
        ...
        <button onClick={() => setTab("areas")} style={tab==="areas"?S.tabA:S.tab}>Áreas</button>
      </div>
      ...
      {tab === "nuevo" ? (
          ...
          {comunicados.map(c => (
            <div key={c.id} style={{...S.adminRow, opacity: c.archivado ? 0.5 : 1}}>
              ...
              <div style={{display:"flex", gap:10}}>
                ...
                <button onClick={() => { if(window.confirm("¿Borrar?")) onSave(comunicados.filter(x => x.id !== c.id), null) }} style={{...S.btnIcon, color:"red"}}>🗑</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{padding:20}}>
          <button onClick={agregarArea} style={S.btnPub}>+ Agregar Nueva Área</button>
...
const S = {
  ...
  chazzText: { fontSize: 26, fontWeight: 900, color: "#333", letterSpacing: "-1px" },
  burgerIcon: { width: 45, height: 45, objectFit: "contain" },
  btnBack: { background: "#666", border: "none", color: "#fff", padding: "6px 10px", borderRadius: 6, fontSize: 12 },
  ...
  badge: { padding: "2px 8px", borderRadius: 20, fontWeight: "bold", textTransform: "uppercase", fontSize: 10 },
